...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh) : data_(new rslidar_rawdata::RawData())
{
  ...
  private_nh.param("subscribe_msop_packets",subscribe_msop_packets,std::string("rslidar_packets"));
  rslidar_scan_ = node.subscribe(subscribe_msop_packets, 10, &Convert::processScan, (Convert*)this,
