...
#include <rslidar_msgs/rslidarDeviceInfo.h>
...
public:
  ...
  void parseDeviceInfo(const uint8_t* data, rslidar_msgs::rslidarDeviceInfo &device_info);
  ...
  bool is_init_angle_;
  int block_num = 0;
  rslidar_msgs::rslidarDeviceInfo device_info_;
private:
  uint8_t GetSubByte(const uint8_t& value, const int32_t start_pos, const int32_t length);
  std::string ByteToHex(const uint8_t& value);
  std::string ByteToHex(const uint32_t& value);
  int32_t parseMotSpeed(const uint8_t* data);

  std::string parseLidarIP(const uint8_t* data);
  std::string parsePCIP(const uint8_t* data);
  std::string parseLidarMACAddress(const uint8_t* data);
  std::string parseMsopLidarSendPort(const uint8_t* data);
  std::string parseMsopPCRecvPort(const uint8_t* data);
  std::string parseDifopLidarSendport(const uint8_t* data);
  std::string parseDifopPCRecvPort(const uint8_t* data);

  int32_t parseMotPhase(const uint8_t* data);

  std::string parseTopFrm(const uint8_t* data);
  std::string parseBotFrm(const uint8_t* data);
  std::string parseSerialNumber(const uint8_t* data);

  std::string parseUTCTime(const uint8_t* data);

  float parseLdat1(const uint8_t* data);
  float parseLdat2(const uint8_t* data);
  float parseVdat12V(const uint8_t* data);
  float parseVdat12VM(const uint8_t* data);
  float parseVdat5V(const uint8_t* data);
  float parseVdat3V3(const uint8_t* data);
  float parseVdat2V5(const uint8_t* data);
  float parseVdat1V2(const uint8_t* data);

  int32_t parseCksumst(const uint8_t* data);
  float parseMancErr1Per(const uint8_t* data);
  float parseMancErr2Per(const uint8_t* data);
  float parseBotTemperature1(const uint8_t* data);
  float parseBottemperature2(const uint8_t* data);
  float parseTopTemperature1(const uint8_t* data);
  float parseTopTemperature2(const uint8_t* data);
  float parseBotTemperature5(const uint8_t* data);

  bool parsePPSLock(const uint8_t* data);
  bool parseGprmcLock(const uint8_t* data);
  bool parseUTCLock(const uint8_t* data);
